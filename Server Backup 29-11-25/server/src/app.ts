...
const start = async () => {
    ...
    // NOTE: registerRoutes moved to AFTER buildAdminRouter (line ~356)
    // This ensures AdminJS registers @fastify/multipart plugin first
    ...
    app.post('/admin/test-auth', async (_request, _reply) => {
        try {
            const { email, password } = (_request as any).body;
    ...
    // CRITICAL: AdminJS must be built BEFORE registerRoutes to ensure multipart plugin is available
    await buildAdminRouter(app);

    // Register custom routes AFTER AdminJS (so multipart plugin is available)
    try {
