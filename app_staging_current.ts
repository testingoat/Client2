...
const start = async () => {
    ...
    // 🔔 Static Assets Route - Serve Bootstrap files locally to bypass CSP
    ...
    // 🔔 FCM Dashboard Route - Moved OUTSIDE /admin/* to bypass AdminJS CSP
    app.get("/fcm-dashboard", async (request, reply) => {
        try {
            const fs = await import('fs');
    ...
    console.log('✅ FCM Dashboard route registered at /fcm-dashboard (bypasses AdminJS CSP)');
...
<body style="font-family: Arial; padding: 20px; background: #1a1a1a; color: white;">
    ...
    app.get("/fcm-dashboard/api/tokens", async (request, reply) => {
        try {
    ...
    app.post("/fcm-dashboard/api/send", async (request, reply) => {
        try {
    ...
    app.get("/fcm-dashboard/api/history", async (request, reply) => {
        try {
    ...
    app.get("/fcm-dashboard/api/stats", async (request, reply) => {
        try {
            const { Seller } = await import('./models/index.js');
            const { NotificationLog } = await import('./models/notificationLog.js');
