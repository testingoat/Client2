...
<body style="font-family: Arial; padding: 20px; background: #1a1a1a; color: white;">
    ...
    // 📁 FCM Dashboard Static Assets Route
    app.get(" /admin/fcm-management/assets/*, async (request, reply) => {
 try {
 const fs = await import(s);
 const path = await import(path);
 
 // Extract the file path from the URL
 const assetPath = request.url.replace(/admin/fcm-management/assets/, \);
 const filePath = path.join(/var/www/goatgoat-staging/server/src/public/fcm-dashboard/assets, assetPath);
 
 // Determine content type based on file extension
 const ext = path.extname(filePath).toLowerCase();
 const contentTypes: Record<string, string> = {
 .css: 	ext/css,
 .js: pplication/javascript,
 .woff: ont/woff,
 .woff2: ont/woff2,
 .ttf: ont/ttf,
 .eot: pplication/vnd.ms-fontobject,
 .svg: image/svg+xml\n };
 
 const contentType = contentTypes[ext] || pplication/octet-stream;
 ...
 reply.status(404).send(Asset not found);
    ...
    app.post("/admin/fcm-management/api/send", async (request, reply) => {
        try {
            ...
            const { title, message, tokens, sellerIds, customerIds, deliveryPartnerIds, targetType, userType } = body;
            ...
            if (targetType === 'tokens' && Array.isArray(tokens)) {
            ...
            } else if (targetType === 'customers' && Array.isArray(customerIds)) {
                // Get tokens for specific customers
                ...
                customers.forEach((customer: any) => {
                    if (customer.fcmToken) {
                        targetTokens.push(customer.fcmToken);
                    }
                });
                ...
                if (targetTokens.length === 0) {
                    ...
                    return {
                        success: false,
                        error: 'No FCM tokens found for specified customers',
                        dryRun: true
                    };
                }
            } else if (targetType === 'delivery' && Array.isArray(deliveryPartnerIds)) {
                // Get tokens for specific delivery partners
                ...
                partners.forEach((partner: any) => {
                    if (partner.fcmToken) {
                        targetTokens.push(partner.fcmToken);
                    }
                });
                ...
                if (targetTokens.length === 0) {
                    ...
                    return {
                        success: false,
                        error: 'No FCM tokens found for specified delivery partners',
                        dryRun: true
                    };
                }
            } else if (targetType === 'all' || targetType === 'all-users') {
                // Get all available tokens from all user types
                const { Customer, Seller, DeliveryPartner } = await import('./models/index.js');
                ...
                customers.forEach((customer: any) => {
                    if (customer.fcmToken) targetTokens.push(customer.fcmToken);
                });
                ...
                // Get delivery partner tokens
                const partners = await DeliveryPartner.find({
                    fcmToken: { $exists: true, $ne: null }
                }).select('fcmToken');
                partners.forEach((partner: any) => {
                    if (partner.fcmToken) targetTokens.push(partner.fcmToken);
                });
            } else if (targetType === 'all-customers') {
            ...
            } else if (targetType === 'all-sellers' || targetType === 'all') {
                // Get all seller tokens
                const { Seller } = await import('./models/index.js');
                const sellers = await Seller.find({
                    'fcmTokens.0': { $exists: true }
                }).select('fcmTokens');
            ...
            } else if (targetType === 'all-delivery') {
                // Get all delivery partner tokens
                const { DeliveryPartner } = await import('./models/index.js');
                const partners = await DeliveryPartner.find({
                    fcmToken: { $exists: true, $ne: null }
                }).select('fcmToken');
                ...
                return {
                    success: false,
                    error: 'Invalid targetType. Must be "tokens", "sellers", "customers", "delivery", "all", "all-users", "all-customers", "all-sellers", or "all-delivery"',
                    dryRun: true
                };
            }
